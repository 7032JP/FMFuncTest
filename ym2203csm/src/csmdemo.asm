...
;
; 【クリーンルーム宣言】
;   本プログラムは公開されている公式資料の記載のみを参照して独自に書き起こした
;   ものであり、既存のソフトウェア (ROM やその他のプログラム) のコードは含まない。
;   参照した公式資料と章番号:
;     - FM-7 ユーザーズマニュアル システム仕様 (富士通)
;         1.2 メモリ、I/O アドレスマップ / 1.6 パレット機能 / 1.9 キーボード インタフェース /
;         1.13 PSG 機能 (レジスタアクセス手順は OPN も共通) / 1.14 メイン－サブ ブロック間インタフェース仕様 /
;         1.20 ミニフロッピィディスク インタフェースカード / 2.2.17 メイン－サブインタフェース解説
;     - F-BASIC 解析マニュアル フェーズIII サブシステム編
;         第1章 サブシステム転送命令の使い方 (TEST コマンド $3F と MOVE/JSR/JMP/END サブコマンド) /
;         第2章 サブシステムのアーキテクチャ (共有 RAM、メイン－サブ間インタフェース)
;     - YM2203 (OPN) のレジスタと CSM モードはヤマハ公開のデータシート / アプリケーションマニュアルの記載に基づく
;     - FM77AV 固有の機能は FM77AV シリーズの公式資料 (ユーザーズマニュアル /
;       ハードウェア解説) の記載に基づく
;       (画面モードレジスタ $FD12、アナログパレット $FD30-$FD34、OPN レジスタ $FD15/$FD16)
;
